...
{	
	...
	static	unsigned char synvalid=1;	//模拟闰秒标志
	if(USART_GetITStatus(USART1,USART_IT_RXNE) != RESET)//检查是否是指定的中断发生
	{
		...
		{
			if((rec_SYNIndex==18)&&synvalid) 					//长度合格	
				{
			...
			else if((rec_SYNIndex==35 )&&synvalid) 			//长度合格	
				{
			#ifndef NTP_Server
...
void handleap(void)//FH1000接收串口时间信息
{
	...
	if(leapNum >= 3)//连续3次闰秒预告
	{
		...
		if(leap61 != 0)//正闰秒
		{
			if((ngettime.seconds%60) == 1)//g_CFL_ucSec2是收到模拟闰秒帧的一个时间每秒会加一次
			{
				timeupdata.tv_sec = -1;
