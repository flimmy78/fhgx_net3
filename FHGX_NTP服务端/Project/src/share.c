...
void offset_time(sysTime *calcTime)
{
	...
	//printf("offn=%d,",calcTime->SubTime.nanoseconds);
...
void abjClock(const TimeInternal subtime)	//同步装置调整时间
{
	...
	if (0 != subtime.seconds || abs(subtime.nanoseconds) > MAX_ADJ_OFFSET_NS )//大于100ms的粗调大时间
	...
	else	//细调时间
		{
			...
			if(abs(offsetNorm) < 500)	//时间偏差绝对小于300
				{				
					objnum++;		
					if(!synflags) 				//未同步
						{
			...
			//myprintf("  %d= %d/%d + %d (%d/%d) ",adj,offsetNorm,Mservo.ap,sTime.observedDrift,offsetNorm,Mservo.ai);
