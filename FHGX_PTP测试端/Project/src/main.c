...
void Read_ConfigData(FigStructData *FigData)//godin flash配置信息读取
{
	...
	if(readflash[0] == 0xffffffff)//默认配置
		{
			...
			FigData->LocalPort = 7861;					//godin 频率信息端口号，组播端口号
	...
	printf("FigData->AnnounceInterval=%d\n",FigData->AnnounceInterval);
	printf("FigData->AnnounceOutTime=%d\n",FigData->AnnounceOutTime);
