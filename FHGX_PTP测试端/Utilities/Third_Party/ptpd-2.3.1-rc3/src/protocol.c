...
extern void My_timerStart(UInteger16 index, UInteger32 interval, IntervalTimer * itimer);		//配置定时器开始工作
				...
				if (ptpClock->waiting_for_first_sync) 		//第一次收到同步报文
					{
						...
						if (ptpClock->delayMechanism == E2E)
							{
								My_timerStart(DELAYREQ_INTERVAL_TIMER,pow2ms(ptpClock->logMinDelayReqInterval),ptpClock->itimer);	//开DelayReq计时器
							}
						else if (ptpClock->delayMechanism == P2P)
							{
								My_timerStart(PDELAYREQ_INTERVAL_TIMER,pow2ms(ptpClock->logMinPdelayReqInterval), ptpClock->itimer);
							}
					} 
